module Dog
  class Command
    ...
    def help(depth = 0, parent_matcher_string = nil)
      ...
      output = "#{"  " * depth}#{@title}: #{matcher_string}\n"
      output += @subcommands.map { |c| c.help(depth + 1, matcher_string) }.inject(:+) unless @subcommands.empty?
      output
    end
  end
end
